...
#[test]
fn is_registered() {
    assert!(DEFAULT_ZARR_CONVENTION_REGISTRY.contains(&License::DEFINITION.id_uuid()));
}
...
fn test_examples(
    ...
    assert!(conventions.contains(&License::DEFINITION.id_uuid()));

    let _ = License::from_attributes_nested(&attrs).expect("should be present");
}
